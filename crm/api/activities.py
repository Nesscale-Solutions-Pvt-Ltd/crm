...
def get_activities(name: str):
	...
	elif frappe.db.exists("HD Ticket", name):
...
def get_ticket_activities(name: str):
	if not frappe.has_permission("HD Ticket", "read", name):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	get_docinfo("", "HD Ticket", name)
	docinfo = frappe.response["docinfo"]
	...
	doc = frappe.db.get_values("HD Ticket", name, ["creation", "owner"])[0]
	activities = [
		{
			"activity_type": "creation",
			"creation": doc[0],
			"owner": doc[1],
			"data": _("created this ticket"),
			"is_lead": False,
		}
	]
	...
	notes = get_linked_notes(name) + get_linked_calls(name).get("notes", [])
	tasks = get_linked_tasks(name) + get_linked_calls(name).get("tasks", [])
	attachments = get_attachments("HD Ticket", name)
